...
@Component({
  selector: "app-allstar",
  templateUrl: "./allstar.component.html",
  styleUrls: ["./allstar.component.scss"]
})
export class AllstarComponent implements OnInit {
  ...
  getAllStarShoes(allShoes) {
    this.allShoes = allShoes.filter(item => {
      return item.brand == "Allstar";
    });
    this.allStarShoes = this.allShoes;
    localStorage.setItem("allStarShoes", JSON.stringify(this.allShoes));
  }

  filterItem(value) {
    this.allShoes = JSON.parse(localStorage.getItem("allStarShoes")).filter(
